...
/////////////////////////////
		int position = 0;
		int next_position = 0;
//identify where it is the start position and end position in the demand scenario data to update
//position: start point; next_position: end point.
...
//split the list based on the start position and end position into two: temp1 and temp2.
//temp1 saves the data list before start point (i.e. position);
//temp2 saves the data list after end point (i.e. next_position);
		if (position != 0)
		{
			temp1 = m_DemandScenarioData.mid(0, position);
			if(next_position < m_DemandScenarioData.count())
			{
				temp2 = m_DemandScenarioData.mid(next_position, -1);
			}
			else
			{
				temp2.clear();
	...
	else
	{
		if (m_DmdScnNo == 1)
		{
			...
			temp2.clear();
			if (m_DmdScnNo = 1)
				demands = m_DemandScenarioData[0];
			else //append it the end of the tree view
				demands = QString("ds %0").arg(m_DmdScnNo+1);
		}
	}
...
//prpeare the list for the updated profiles 	
	for (int i=0; i < rowCount+1; i++)
	{
		if (i==0)
		{
			listDemands.append(demands);
		}
		else
		{
			demands = " " + dsSelected->listSelectedDemandWidget->item(i-1)->text();
			listDemands.append(demands);
		}
	}
	m_DemandScenarioData.clear();

//make the new final list 
	if (!temp1.empty()&&!temp2.empty())
		m_DemandScenarioData = temp1 + listDemands + temp2;
	else if(!temp1.empty() && temp2.empty())
		m_DemandScenarioData = temp1 + listDemands;
	else if(temp1.empty() && !temp2.empty())
		m_DemandScenarioData = listDemands + temp2;
	else
		m_DemandScenarioData = listDemands;
...
//make the new list using the data from the selected list widget 
//and append it to the end of model data list.
	for (int i=0; i < rowCount+1; i++)
	{
		if (i==0)
		{
			m_DmdScnNo++;
			demands = QString("ds %0").arg(m_DmdScnNo);
			m_DemandScenarioData.append(demands);
		}
		else
		{
			demands = " " + dsSelected->listSelectedDemandWidget->item(i-1)->text();
...
//temp1 saves the data list before start point (i.e. position);
//temp2 saves the data list after end point (i.e. next_position);
	if (position != 0) //if it is not the first demand scenario
	{
		
		temp1 = m_DemandScenarioData.mid(0, position);
		//check how many demand scenario witin temp1
		for (int p = 0; p < temp1.count(); p++)
		{
			if(!temp1[p].startsWith(" "))
			{
				noScenario++;
			}
		}
		...
		{
			temp2 = m_DemandScenarioData.mid(next_position, -1);
			//check how many demand scneario witin temp2
			//reconstruct the sequence of the demand scneario in temp2 based on temp1
			for (int q=0; q < temp2.count(); q++)
			{
				if(!temp2[q].startsWith(" "))
				{
					temp2[q] = QString("ds %1").arg(noScenario++);
				}
			}
		}
		else
	...
	else //if it is the first demand scenario
	{
		temp1.clear();
		noScenario = 1;
		if(next_position < m_DemandScenarioData.count())
...
//clear m_DemandScenarioData list to prepare for reassigning the data
	m_DemandScenarioData.clear();

//make the new final list 
	if (!temp1.empty()&&!temp2.empty())
		m_DemandScenarioData = temp1 + temp2;
	else if(!temp1.empty() && temp2.empty())
		m_DemandScenarioData = temp1 ;
	else if(temp1.empty() && !temp2.empty())
		m_DemandScenarioData = temp2;
	else
		m_DemandScenarioData = temp1; //or temp2, it is the same as both of them are empty
...
//if the current index is the parent item
	...
	for (int i=1; i < temp.size(); i++)
	{
		...
		{
			temp[i].remove(0,1);
			dsSelected->listSelectedDemandWidget-> insertItem(i, temp[i]);
		}
	}	
}
