    ...
    QwtPlot( parent )
{
   ...
   // setTitle( "Temperature of Friedberg/Germany" );
    ...
    const int numHours = 8760;
    QVector<QPointF> TemperatureData( numHours );
    QVector<QPointF> DirectSolarData( numHours );

//    QVector<QwtIntervalSample> rangeData( numDays );

    for ( int i = 0; i < numHours; i++ )
    {
        const ContextData &t = friedberg2007[i];
		
		TemperatureData[i] = QPointF( double( i ), t.m_pDbTemperature[i] );
		DirectSolarData[i] = QPointF( double( i ), t.m_pDirectSolar[i] );

//        rangeData[i] = QwtIntervalSample( double( i ),
//            QwtInterval( t.minValue, t.maxValue ) );
    }

    insertTemperatureCurve( QString( "Temperature [%1C]" ).arg( QChar( 0x00B0 ) ), TemperatureData, Qt::black );
    insertDirectSolarCurve( QString( "Direct Solar [W/m2]" ).arg( QChar( 0x00B0 ) ), DirectSolarData, Qt::red );
...
{
    const int days[] = { 744, 672, 744, 720, 744, 720, 744, 744, 720, 744, 720, 744 };
...
void Plot::insertTemperatureCurve( const QString& title,
...
void Plot::insertDirectSolarCurve( const QString& title,
    const QVector<QPointF>& samples, const QColor &color )
{
    d_curve = new QwtPlotCurve( title );
    d_curve->setRenderHint( QwtPlotItem::RenderAntialiased );
